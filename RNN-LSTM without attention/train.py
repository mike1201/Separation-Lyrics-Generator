...
def main():
    ...
    parser.add_argument('--nwords', type = int, default = 50000, help = 'number of words')
...
def train(args):
    # 3-1
    data_loader = TextLoader(args.data_dir, args.batch_size, args.seq_length, args.nwords, args.input_encoding) # from utils.py
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for e in range(model.epoch_pointer.eval(), args.num_epochs):
            sess.run(tf.assign(model.lr, args.learning_rate * (args.decay_rate ** e))) # Set decay rate, model.lr = learning rate
